...
use macroquad::{input::{self, utils::*}, miniquad::EventHandler};
use super::converter::{
    mq_to_bevy_keycode,
    mq_to_bevy_mbtn,
    mq_to_bevy_tch
...
impl EventHandler for EHandler {
    ...
    fn draw(&mut self) {
        self.collected.events.push(MQEvent::Draw(window::RequestRedraw));
    }
    ...
    fn mouse_motion_event(&mut self, x: f32, y: f32) {
        ...
        self.collected.events.push(MQEvent::CursorMotion(window::CursorMoved {
            window: self.window,
            position: Vec2::new(mpos.0, mpos.1),
        }));

        self.collected.events.push(MQEvent::MouseMotion(mouse::MouseMotion { 
            delta: Vec2::new(x, y)
        }));
    }
    ...
    fn key_down_event(&mut self, 
        ...
        self.collected.events.push(MQEvent::Keyboard(keyboard::KeyboardInput {
            window: self.window,
    ...
    fn mouse_button_down_event(&mut self, btn: input::MouseButton, _x: f32, _y: f32) {
        self.collected.events.push(MQEvent::MouseButton(mouse::MouseButtonInput {
            window: self.window,
    ...
    fn mouse_wheel_event(&mut self, x: f32, y: f32) {
        self.collected.events.push(MQEvent::MouseWheel(mouse::MouseWheel {
            window: self.window,
    ...
    fn touch_event(&mut self, 
        _phase: macroquad::miniquad::TouchPhase, 
        id: u64, x: f32, y: f32
    ) {
        self.collected.events.push(MQEvent::Touch(touch::TouchInput {
...
pub fn init_events(w: &mut World) {
    ...
    let win_ent = w.query_filtered::<Entity, With<PrimaryWindow>>()
        .get_single(w).expect("Failed to get PrimaryWindow's entity");
    w.insert_non_send_resource(EHandler::new(win_ent));
}

pub fn get_events(w: &mut World) {
    let sub = w.get_resource::<InputSubscriber>().expect("Can't get event subscriber").0;
    let mut collected = {
        let mut handler = w.get_non_send_resource_mut::<EHandler>().expect("can't get event handler");
